...
\begin{equation*}
\frac{1}{c} \fint_{B_j} f\,d\mu\le \fint_{\mathcal{B}_j} f\,d\mu \le \alpha < \fint_{B_j} f\,d\mu, \quad j\geq 1;
\end{equation*}
...
\begin{equation*}
 \fint_{B_j} f\,d\mu\le \frac{\mu(\mathcal{B}_j)}{\mu(B_j)} \fint_{\mathcal{B}_j} f\,d\mu \le c \fint_{\mathcal{B}_j} f\,d\mu,
\end{equation*}
а неравенство $f(x)\le \alpha$ вытекает из теоремы Лебега о дифференцировании интегралов.
...
\begin{equation*}
\mu\{x \in B_0:\ |f(x)-I_{B_0}^{\varphi}|>\lambda\} \le A \mu(B_0) \exp(-a\lambda), \quad \lambda>0.
\end{equation*}
\textit{Здесь}
\begin{equation*}%\label{eq}
a = \frac{\ln \Lambda}{ \varphi^{-1}(a_\varphi(1+c \Lambda)  \varphi(\|f\|^{*}_\varphi))},\quad A=c \Lambda,
\end{equation*}
\textit{где $c$ ---  постоянная из леммы 2 \ref{lmCalderonZygmundDecomposition}}.
...
\end{equation*}
\begin{equation*}%\label{eq}
\le  a_\varphi \left(\varphi({\|f\|^{*}_\varphi}) +  c  \Lambda \varphi(\|f\|^{*}_\varphi)\right)= a_\varphi \varphi(\|f\|^{*}_\varphi)(1+c \Lambda) .
\end{equation*}
...
\Lambda \varphi(\|f\|^{*}_\varphi) < \fint_{B_{s,j}^{(k)}} \varphi(|f(x)-I_{B_j^{(k-1)}}^{\varphi}|)\,d\mu(x) \le  c \Lambda \varphi(\|f\|^{*}_\varphi), \quad s\geq 1
\end{equation}
и
...
\le \varphi^{-1}(\Lambda\|f\|^{*}_\varphi) + (k-1)\varphi^{-1} \left( a_\varphi (1+c \Lambda) \varphi(\|f\|^{*}_\varphi)\right)=k \varphi^{-1}\left( a_\varphi (1+c \Lambda) \varphi(\|f\|^{*}_\varphi)\right),
\end{equation}
...
\begin{equation*}
k \varphi^{-1} \left( a_\varphi (1+c\Lambda) \varphi(\|f\|^{*}_\varphi)\right) \le \lambda < (k+1) \varphi^{-1} \left( a_\varphi (1+c\Lambda) \varphi(\|f\|^{*}_\varphi)\right)
\end{equation*}
...
\end{equation*}
\begin{equation*}
\le \sum_{j\geq1} \mu(\mathcal{B}_{j}^{(k)}) \le  c \sum_{j\geq 1} \mu(B_{j}^{(k)}) \le  c \left(\frac{1}{\Lambda}\right)^{k} \mu(B_0) = c \mu(B_0)\exp\left(-k \ln \Lambda\right) \le
\end{equation*}
\begin{equation*}
\le  c \mu(B_0)\exp\left(\left(-\frac{\lambda}{ \varphi^{-1} \left(a_\varphi (1+c\Lambda) \varphi(\|f\|^{*}_\varphi) \right)}\right)\ln \Lambda\right)
\end{equation*}
\begin{equation*}
...
\end{equation*}
\begin{equation*}
\le c \mu(B_0)\exp\left(-a \Lambda \right) \exp \left( a \varphi^{-1}\left( a_\varphi (1+c\Lambda) \varphi(\|f\|^{*}_\varphi)\right)\right)
=  c \Lambda \mu(B_0)\exp\left(-a \lambda\right).
\end{equation*}
Положив $A =  c \Lambda$ получим утверждение теоремы.

Очень похожие результаты были  получены в работе [6]. В частности, для любой возрастающей непрерывной положительной функции   $\varphi: \mathbb{R}^+\to \mathbb{R}^+$, такой что $\lim_{t\to \infty} \varphi(t)= \infty$ и удовлетворющей условию
...
\textbf{Теорема 2.}
\textit{Если $\varphi$ удовлетворяет вышеуказанном условиям, шар $B_0 \subset X$, $f \in \widehat{BMO}_\varphi(B_0)$. Тогда существуют постоянные $K$ и $k(f)$ такие, что}
\begin{equation*}
\mu\{x \in B_0:\ |f(x)-I_{B_0}^{\varphi}|>\lambda\} \le K \mu(B_0) \exp(-\frac{\lambda}{k(f)}), \quad \lambda>0.
\end{equation*}

Выписать в явном виде постоянные $K$ и $k(f)$ довольно проблематично, поэтому утверждать какая из оценок более точная не представляется возможным.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{equation*}
\begin{equation*}
\le \sup\left\{y>0:\ A\mu(B_0)\exp(-a y)>t\right\}
\end{equation*}
\begin{equation*}
...
\sup_{B\subset B_0}\fint_B |f(x)-f_B|\,d\mu(x) \le \frac{2 \left(1 + \ln (c \Lambda)\right)}{\ln \Lambda} \varphi^{-1}\left((1+c \Lambda)  \varphi(\|f\|^{*}_\varphi)\right)
\end{equation}
...
\le  \frac{2}{a}\fint_0^{\mu(B)} \ln\frac{A\mu(B)}{t}dt = \frac{2A}{a}\int_0^{1/A} \ln\frac{1}{t}dt=\frac{2}{a} (\ln A +1).
\end{equation}
Таким образом,
\begin{equation*}
\sup_{B\subset B_0}\fint_B |f(x)-f_B|\,d\mu(x) \le \frac{2}{a}(\ln A +1) = \frac{2 \left(1 + \ln (c \Lambda)\right)}{\ln \Lambda} \varphi^{-1}\left(a_\varphi(1+c \Lambda)  \varphi(\|f\|^{*}_\varphi)\right)< + \infty.
\end{equation*}
...
Это утверждение вытекает из следствия 2 по аналогии с работой [7].
...
\begin{equation*}
\mu_1(f,t) \equiv \sup_{\substack{B\subset B_0\\ 0<r_B\le t}} \inf_{c\in \mathbb{R}} \left(\fint_B |f(x)-f_B| \,d\mu(x)\right).
\end{equation*}
Это вытекает из очевидных неравенств
\begin{equation*}
\fint_B |f(x)-f_B|\,d\mu(x) \le 2\fint_B |f(x)-I_{B}^{\varphi}|\,d\mu(x),
\end{equation*}
\begin{equation*}
\fint_B |f(x)-I_{B}^{\varphi}|\,d\mu(x) \le \fint_B |f(x)-f_B|\,d\mu(x).
\end{equation*}
...
\textbf{Доказательство.} Зафиксируем $0<t\le r_{B_0}$ и пусть $B \subset B_0$, $r_B\le t$.
 ...
 Пусть $h=f|_{B}$ -- сужение функции $f$ на шар $B$. Тогда $h \in \widehat{BMO}_\varphi(B)$, $\|h\|^{*}_\varphi\le \|f\|^{*}_\varphi$. Обозначим $g(x)=|h(x)-I_{B}^{\varphi}|$.
...
Доказательство этой леммы можно найти в работе [9].
...
%%\le c_1 \left(\frac{r_2}{r_1}\right)^{\gamma}.
%%\end{equation*}

\textbf{Теорема 3.}\label{thEsimateOfModulusByMu}
...
\fint\limits_{B_1} |f - f_{B_2} | \,d\mu \le \frac{\mu(B_2)}{\mu(B_1)}\bigg(\fint\limits_{B_2} |f - f_{B_2}| \,d\mu\bigg) \le \beta \fint\limits_{B_2} |f - f_{B_2}| \,d\mu,
\end{equation}
...
\begin{equation*}
\le  \left(\fint\limits_{B(x, r)} |f(z) - f_{B(x, r)}|\,d\mu(z) + c_1 \fint\limits_{B(y, 2 r)} |f(z) - f_{B(y, 2 r)}|\,d\mu(z) \right) \le c_2 \mu_{1}(f,2 r).
\end{equation*}
Таким образом,
\begin{equation*}
|f(x) - f(y)|\le (1+\beta)\left(\int \limits_{0}^{r} \mu_{1}(f,t)\,\frac{d t}{t} + c_2\mu_{1}(f,2 r) +\int\limits_{0}^{2 r} \mu_{1}(f,t)\,\frac{d t}{t}\right)\le
\end{equation*}

\begin{equation*}
\le (1+\beta)\left(\int\limits_{0}^{r} \mu_{1}(f,t)\,\frac{d t}{t} + c_2 \int\limits_{r}^{2 r} \mu_{1}(f,t)\,\frac{d t}{t} +\int\limits_{0}^{2 r} \mu_{1}(f,t)\,\frac{d t}{t}\right) \le
\end{equation*}
\begin{equation*}
 \le (1+c_2) (1+\beta) \int\limits_{0}^{2 r} \mu_{1}(f,t)\,\frac{d t}{t} \le M \int\limits_{0}^{2 r}  \varphi^{-1} (a_\varphi(1+c\Lambda) \varphi(\mu_{\varphi}(f,t)))\,\frac{d t}{t} < \infty.
\end{equation*}

Так как множество точек Лебега для $f$ плотно в $X$, то $f$ можно единственным образом продолжить с множества точек Лебега на все пространство $X$ до функции $g$ с сохранением неравенства \eqref{eqEsimateOfModulusByMu}
\begin{equation*}
...
Сравним теорему 3 с предшествующими результатами. В частном случае $t^p$, $p=1$, $X=Q$ --- куб в $\mathbb{R}^n$ ее утверждение было получено другим, значительно более сложным способом в работе С.~Спанне [3]. Теорема 3 для функций $\varphi(t)=t^p$ при $0<p\le 1$ дает усиление этого результата, так как условие (\ref{eqEsimateOfModulusByMu}) тем сильнее, чем меньше $p$.

Теорема 3 усиливает также основной результат работы [10], дающий описание классов Гельдера в терминах поведения средних Стеклова.
...
\sup_{B:r_B<R_0}\frac{1}{\eta(r_B)}\left(\fint_{B}|f-f_B|^p\,d\mu\right)^{1/p}<+\infty,
  \end{equation}
то $f$ эквивалентна некоторой функции класса $H^{\eta}(X)$. В частном случае $p=1$, $\eta(t)=t^{\alpha}$ и $X\subset\mathbb{R}^n$ этот результат --- классическая теорема К.Кампанато--Н.Мейерса [4], [5] (см. также более поздний результат из [11]).

Этот результат является простым следствием из теоремы 3, причем без дополнительного условия $f\in L^1_{\mathrm{loc}}(X)$. Действительно, в силу условия (\ref{eqCampanatoCondition}) $\mu_p(f,t) \lesssim \eta(t)$. Интегрируя это неравенство по $t$ и используя (\ref{eqEsimateOfModulusByMu}), получим
\begin{equation*}
